...
static const CGFloat kHorizontalAdjustment = 4.0f;
...
- (void)setupUI {
    self.shape = ZYSwitchShapeTypeOval;
...
- (void)setShape:(ZYSwitchShapeType)newShape
{
    if (_shapeType != newShape)
        _shapeType = newShape;
    
    if (newShape == ZYSwitchShapeTypeOval)
    {
        [self.onBackgroundView.layer setCornerRadius:self.frame.size.height/2];
        [self.offBackgroundView.layer setCornerRadius:self.frame.size.height/2];
        [self.thumbView.layer setCornerRadius:(self.frame.size.height-kHorizontalAdjustment)/2];
    }
    else if (newShape == ZYSwitchShapeTypeRectangle)
    {
    ...
    else if (newShape == ZYSwitchShapeTypeRectangleNoCorner)
    {
